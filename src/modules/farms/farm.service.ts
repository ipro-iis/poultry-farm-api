...
export function create(companyId: string, farmNo: string, remark: string) {
  return prisma.farm.create({ data: { companyId, farmNo, remark } });
}

export function update(id: string, data: { farmNo?: string; remark?: string; status?: FarmStatus }) {
...
export async function closeFarm(id: string, companyId: string) {
...
export async function removeFarm(id: string, companyId: string) {
  // 1️⃣ Verify farm exists and belongs to the company
  const farm = await prisma.farm.findUnique({ where: { id } });
  if (!farm || farm.companyId !== companyId) {
    throw new Error("Not found");
