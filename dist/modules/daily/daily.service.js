...
//   // });
// }
export async function listByFarm(farmId) {
...
export async function updateAndRecalculate(id, data) {
    return prisma.$transaction(async (tx) => {
        ...
        let prevQty = updatedDay.chicken?.qty ?? 0;
        let prevWeight = updatedDay.chicken?.weight ?? 0;
        // 4. Loop through subsequent days and recalc
        for (const day of subsequentDays) {
            const lossQty = (day.deathLoss?.qty ?? 0) + (day.deathSale?.qty ?? 0) + (day.discardSale?.qty ?? 0) + (day.goodSale?.qty ?? 0);
            const lossWeight = (day.deathLoss?.weight ?? 0) + (day.deathSale?.weight ?? 0) + (day.discardSale?.weight ?? 0) + (day.goodSale?.weight ?? 0);
